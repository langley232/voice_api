...
import requests
from streamlit_webrtc import AudioRecorder # Using AudioRecorder as planned
...
# The AudioRecorder returns audio bytes when the recording is stopped.
...
if audio_bytes:
    ...
    if st.button("Transcribe Recorded Audio", key="transcribe_button_audio"):
        ...
        try:
            ...
            stt_response = requests.post(f"{API_URL}/stt", files=files, timeout=60) # Increased timeout
            if stt_response.status_code == 200:
                ...
                st.success(f"Transcription successful! Detected Language: {detected_lang_display}")
                st.session_state.text_to_translate_input = transcribed_text_from_audio # Pre-fill text area
                ...
                if source_language != detected_lang_code and detected_lang_code in lang_codes:
                    ...
                    st.info(f"Source language automatically updated to: {detected_lang_display}")
...
if st.button("Translate", key='translate_button'):
    ...
    elif text_input: # Text from text_area (could be from STT or manually typed)
    ...
    if not final_text_to_translate:
        ...
        try:
            ...
            translate_response = requests.post(f"{API_URL}/translate", json=payload, timeout=60) # Increased timeout
            if translate_response.status_code == 200:
                ...
                st.error(f"Translation Error: {translate_response.status_code} - {translate_response.text}")
        except requests.exceptions.RequestException as e:
            st.error(f"Translation Request failed: {e}")
        except Exception as e_gen:
            st.error(f"An unexpected error occurred during translation: {e_gen}")
...
tts_text_input = st.text_area("Enter text for TTS:", key='tts_text', height=100)
...
if st.button("Generate Speech", key='tts_generate_button'):
    if not tts_text_input.strip():
        ...
        try:
            ...
            tts_response = requests.post(f"{API_URL}/tts", json=payload, timeout=120) # Increased timeout for potentially long generation

            if tts_response.status_code == 200:
                audio_content_bytes = tts_response.content # Raw bytes of the WAV file
                
                if tts_output_option == "Play audio":
                    st.audio(audio_content_bytes, format="audio/wav", start_time=0)
                    ...
                    st.success("Speech generated! Click the button above to download.")
            else:
                try:
                    error_detail = tts_response.json().get("detail", tts_response.text)
                except ValueError: # If response is not JSON
                    error_detail = tts_response.text
                st.error(f"TTS Generation Error: {tts_response.status_code} - {error_detail}")

        except requests.exceptions.Timeout:
            st.error("TTS Request timed out. The model might be taking too long to respond.")
        except requests.exceptions.RequestException as e:
            st.error(f"TTS Request failed: {e}")
        except Exception as e_gen:
            st.error(f"An unexpected error occurred during TTS generation: {e_gen}")
...
asr_audio_bytes_recorder = AudioRecorder(
    pause_threshold=2.0, 
    sample_rate=16000, 
...
if st.button("Transcribe Speech", key='asr_transcribe_button'):
    if not final_asr_audio_bytes:
        ...
        files = {'audio_file': ('asr_audio.wav', final_asr_audio_bytes, 'audio/wav')}
        
        try:
            stt_response = requests.post(f"{API_URL}/stt", files=files, timeout=90) # Increased timeout
            ...
            if stt_response.status_code == 200:
                ...
                st.write(f"**Detected Language:** {detected_language_display} ({detected_language_code})")
                st.text_area("Transcribed Text:", value=transcribed_text, height=200, key='asr_transcribed_output', disabled=False)
            else:
                try:
                    error_detail = stt_response.json().get("detail", stt_response.text)
                except ValueError:
                    error_detail = stt_response.text
                st.error(f"STT/ASR Error: {stt_response.status_code} - {error_detail}")

        except requests.exceptions.Timeout:
            st.error("STT/ASR Request timed out. The model might be taking too long or the audio is too long.")
        except requests.exceptions.RequestException as e:
